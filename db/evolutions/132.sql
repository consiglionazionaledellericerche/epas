...
INSERT INTO roles (name, version) VALUES ('registryManager', 0);

INSERT INTO roles_history (id, _revision, _revision_type, name) 
SELECT id, (SELECT MAX(rev) AS rev FROM revinfo), 0, name  FROM roles WHERE name = 'registryManager';

INSERT INTO users_roles_offices (office_id, role_id, user_id, version)
SELECT uro.office_id, (SELECT id FROM roles WHERE name = 'registryManager'), uro.user_id, 0 
...
LEFT JOIN roles r ON r.id = uro.role_id WHERE r.name = 'registryManager'; 
