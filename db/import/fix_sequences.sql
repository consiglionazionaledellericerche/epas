...
SELECT SETVAL('public.seq_contract_year_recap', COALESCE(MAX(id), 1), MAX(id) IS NOT NULL ) FROM public.contract_year_recap;
...
SELECT SETVAL('public.seq_initialization_absences', COALESCE(MAX(id), 1), MAX(id) IS NOT NULL ) FROM public.initialization_absences;
SELECT SETVAL('public.seq_initialization_times', COALESCE(MAX(id), 1), MAX(id) IS NOT NULL ) FROM public.initialization_times;
...
SELECT SETVAL('public.seq_stamp_profiles', COALESCE(MAX(id), 1), MAX(id) IS NOT NULL ) FROM public.stamp_profiles;
