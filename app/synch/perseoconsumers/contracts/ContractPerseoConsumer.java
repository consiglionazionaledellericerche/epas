...
public class ContractPerseoConsumer {
  ...
  private static final String ALL_DEPARTMENT_CONTRACTS_FOR_EPAS_ENDPOINT = 
      Play.configuration.getProperty("perseo.rest.allcontractsindepartmentforepas");
  
  private static final String CONTRACT_FOR_EPAS_ENDPOINT = 
      Play.configuration.getProperty("perseo.rest.contractforepasbyperseoid");
  ...
  private Optional<String> perseoContractJson(Long perseoId) {
    ...
    String endPoint = URL_BASE + CONTRACT_FOR_EPAS_ENDPOINT + perseoId;
  ...
  private Optional<String> perseoContractsJson(Long perseoId) {
    ...
    String endPoint = URL_BASE + ALL_DEPARTMENT_CONTRACTS_FOR_EPAS_ENDPOINT + perseoId;
    HttpResponse restResponse = WS.url(endPoint).get();
  ...
  private List<PerseoContract> getAllPerseoDepartmentContracts(Long perseoId) {
    ...
    try {
      perseoContracts = new Gson()
          .fromJson(json.get(), new TypeToken<List<PerseoContract>>(){}.getType());
    } catch (Exception exp) {
  ...
  private Optional<PerseoContract> getPerseoContractByPerseoId(Long perseoId) {
    ...
    try {
      perseoContract = new Gson()
          .fromJson(json.get(), new TypeToken<PerseoContract>(){}.getType());
    } catch (Exception exp) {
...
//  }
   ...
   * Tutti i contratti attivi del department con quel perseoId.<br>
   * I contratti di persone non sincronizzate vengono scartati.<br>
   * Formato mappa: person.perseoId -> contract
   * @param perseoDepartmentId department perseo id
   * @param departmentSyncrhonizedPeopleByPerseoId  mappa perseoId -> person
   * @return mappa
   */
  ...
  /**
   * La lista di tutti i contratti attivi delle persone sincronizzate con perseo.
   * La chiave è il person.perseoId.
   * @param office
   * @return
   */
